...
1. Before calculation, the following data must be available:
   ...
   - the azel files (from AZimuth and ELevation), containing information about the observations which shall be ray-traced, have to be stored in *DATA/AZEL/* without subdirectories. When performing ray-tracing for uniformly distributed azimuths (see option *-creatUniAzel* in Section "Input arguments to the ray-tracer"), however, the azel-files will be created automatically by RADIATE and therefore do not have to be available beforehand.
