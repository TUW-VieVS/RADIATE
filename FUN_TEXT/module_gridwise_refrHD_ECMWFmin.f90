! module_gridwise_refrHD_ECMWFmin.f90

!****************************************************************************
!
! PURPOSE:  Module containing subroutine to calculate the refractive index profiles for a grid
!            
!            "gridwise_refrHD_ECMWFmin" is a subroutine to calculate the
...
! History:
! 
! 20.01.2015: create the Fortran-file based on the Matlab subroutine "calc_refr_ind_HD_slim_gridwise_global_ECMWFmin.m"
! 21.01.2015: programming
! 22.01.2015: programming
! 26.01.2015: programming
! 27.01.2015: programming
...
! 04.02.2015: comments
! 12.02.2015: comments
! 17.06.2015: catch error if last height level in "h_lev_lowest2ECMWF" is below the last but one
!             ECMWF pressure level and add a break condition for the loop over the pressure levels
...
module module_gridwise_refrHD_ECMWFmin

contains

    subroutine gridwise_refrHD_ECMWFmin( min_stat_h, &
                                         lat_grid, &
                                         grid_size, &
                                         nr_pres_lev, &
                                         gph, &
        ...
        interpolation_loop: do level= 1, nr_pres_lev-1 ! only till nr_pres_lev-1, because level+1 is used in the loop for the upper level
            ...
            inner_interpolation_loop: do while (h_lev_lowest2upper_limit(ind_interp) <= h_ell_upper_lev_mean) ! last for-loop delivers mean value of last ECMWF pressure level as value for h_ell_upper_lev_mean
                ...
                rho_w_int= wvpr_int(ind_interp, :, :) / ( Rw * T_int(ind_interp, :, :) ) ! in [hPa*s^2/m^2]
    ...
    end subroutine gridwise_refrHD_ECMWFmin
    
end module module_gridwise_refrHD_ECMWFmin