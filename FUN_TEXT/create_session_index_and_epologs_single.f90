! create_session_index_and_epologs_single.f90

!****************************************************************************
!
! PURPOSE:  Subroutine to create epolog-files and session index file fo a specific azel-file (session)
!           Assignment mode to epologs: single
!            
!           "create_session_index_and_epologs_single" is a subroutine to create
...
!           observation of the specific azel-file once.
...
! History:
! 
! 02.12.2014: create the Fortran-file based on the Fortran- and Matlab-file "create_session_index_and_epologs_single"
...
! 10.09.2015: change to epolog1 and session_index1 instead of epolog and session_index and use constants for the length determination
...
! 11.01.2016: correct comment line in session index file
...
subroutine create_session_index_and_epologs_single( load_path_azel, &
                                                    load_filename_azel, &
    ...
    ! define variable for storing the file unit of the epolog files (unit is always the same)
    ...
    ! The epochs are 0, 6, 12 and 18h for each day (grib-file)
    ! The intervalls for the observations belonging to a specific epoch are [epoch-3h, epoch+3h[. The
    ! selection is done using the mjd-day and the observation hour.
    
    !    --> epologs:
    !                    epolog1_0h...... contains observations between [21h, 3h[; attention: observations before 0h have a different date than the grib-file epoch (-1 day)!
    !                    epolog1_6h...... contains observations between [3h, 9h[
    !                    epolog1_12h..... contains observations between [9h, 15h[
    !                    epolog1_18h..... contains observations between [15h, 21h[
    !    
    !                    epolog1+1_0h.... 
    ...
    epochname_loop: do curr_mjd_integer= first_mjd_integer, last_mjd_integer + 1 ! add. +1 necessary for the splitted interval at epoch 0h (and actually only at epoch0): ]18h,24h[ ,[0h,6h[, if last obs. in azel-file are in the interval ]18h,24h[ --> indexing for next day
        ...
        do i= 1, nr_epochs_per_day
            ...
            name_epoch(index_mjd,i)= 'epolog1_' // session_name // '_'  // curr_date_str // epoch(i)
        end do
    ...
    curr_mjd_loop: do curr_mjd_integer= first_mjd_integer, last_mjd_integer
        ...
        obs_loop: do i= 1, nr_data_lines
            ...
            if (floor(all_obs(i) % mjd) == curr_mjd) then
                ...
                ! assign data to epoch at 0h for observations in the interval [0h,3h[
                if (all_obs(i) % hour >= 0 .AND. all_obs(i) % hour < 3) then
                ...
                ! assign data to epoch at 6h for observations in the interval [3h,9h[
                if (all_obs(i) % hour >= 3 .AND. all_obs(i) % hour < 9) then
                ...
                ! assign data to epoch at 12h for observations in the interval [9h,15h[
                if (all_obs(i) % hour >= 9 .AND. all_obs(i) % hour < 15) then
                ...
                ! assign data to epoch at 18h for observations in the interval [15h,21h[
                if (all_obs(i) % hour >= 15 .AND. all_obs(i) % hour < 21) then
                ...
                ! assign data to epoch at 0h for observations in the interval [21h,0h[
                if (all_obs(i) % hour >= 21 .AND. all_obs(i) % hour < 24) then
    ...
    ! check if the number of assignments is not the same as the number of observations since each
    ! observation should be assigned to one epoch
    ! note: sum() without dimension specification delivers sum of all elements
    if ( sum(count_epoch) /= nr_data_lines ) then
    ...
    do i= 1, size(count_epoch,1)
        ...
        do j= 1, size(count_epoch,2)
            ...
            if (count_epoch(i, j) > 0) then
                ...
                write(unit= file_unit_epolog, fmt= '(a)') '% epolog1 for session:'
                ...
                write(unit= file_unit_epolog, fmt= '(a)')'% epolog1 for epoch:'
    ...
    save_filename_session_index= 'session_index1_' // session_name // '.txt'
    ...
    ! first character in the line containing the session name is set to "S" as to faciliate the finding of the session name later when the file is read in
    ! 1x outputs one blank
    write(unit= file_unit_si, fmt= '(a)') '% session index1 for session:'
    ...
    ! ss supresses plus sign output for all following formats
    ! first character in the line containing the number of observations set to "N" as to faciliate the finding of the number later when the file is read in
    write(unit= file_unit_si, fmt= '(a)') '% total number of epologs1 necessary for processing this session:'
...
end subroutine create_session_index_and_epologs_single