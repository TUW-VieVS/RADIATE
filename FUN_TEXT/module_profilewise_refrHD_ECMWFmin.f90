! module_profilewise_refrHD_ECMWFmin.f90

!****************************************************************************
!
! PURPOSE:  Module containing subroutine to calculate the refractive index profiles for a grid (profilewise approach)
!            
!            "profilewise_refrHD_ECMWFmin" is a subroutine to calculate the
...
!            
!            The interpolation (increase of vertical resolution) is done separately for each profile
!            (for each vertical profile based on one horizontal grid point) until the minimum height of the
!            ECMWF uppermost pressure level. Above a standard atmosphere model is used for extrapolation of
!            the meteorological parameters and the extrapolation can therefore be done for all grid points at
!            one level at once.
!            
...
!         n_h_int.................. interpolated/extrapolated hydrostatic refractive index for the
!                                   height levels
!         n_w_int.................. interpolated/extrapolated wet refractive index for the height
!                                   levels
!         n_total_int.............. array containing the calculated total refractive index for all
...
! History:
! 
! 28.01.2015: create the Fortran-file based on the Matlab subroutine "calc_refr_ind_HD_slim_profilewise_global_ECMWFmin.m"
!             and the Fortran version of the gridwise approach
! 29.01.2015: change to module as to save explicit interface block
!             use modules for subroutines with need of explicit interface (module_gph2horth)
!             rename module and subroutine
!             transfer constant "gn" to "module_constants"
!             transfer meteorological constants to "module_meteo_constants"
! 03.02.2015: add output of " nr_h_lev_lowest2upper_limit"
! 04.02.2015: comments
! 12.02.2015: comments
! 02.06.2015: drag "wvpr_extrap= 0" out of loop for extrapolation as this assignment needs to be done only once
! 17.06.2015: correct comments
! 09.09.2015: change calculation of gravity for pressure interpolation to strict formula
! 21.09.2015: remove unnecessary code
! 06.10.2015: enhance code, so that only those vertical levels are interpolated that are really needed according to the
!             minimal station height for which ray-tracing will be done later
! 07.10.2015: optimize calculation of g_prep by reducing the number of calculations to a minimum as it is only dependent on the latitude
! 14.03.2016: correct comment
! 07.03.2018: exception added, if the wvpr from the logarithmic interpolation is unrealistically high (>50 hPa), then perform a linear interpolation instead
!
!****************************************************************************

module module_profilewise_refrHD_ECMWFmin

contains

    subroutine profilewise_refrHD_ECMWFmin( min_stat_h, &
                                            lat_grid, &
                                            grid_size, &
                                            nr_pres_lev, &
                                            gph, &
        ...
        ! note: Because lat_grid contains identical columns in case of the second dimension of longitude,
        !       it is sufficient to transform only the first column of latitudes to [rad] as this will be only used
        !       for the calculation of g_prep.
        ...
        double precision, dimension(:), allocatable :: lat_vec_rad
        ...
        ! loop/index variables for the latitude values of the grid
        integer :: ind_lat
    
        ! loop/index variables for the longitude values of the grid
        integer :: ind_lon
        ...
        double precision :: g_upper
    
        ! define variable for storing geopotential height for lower level
        double precision :: gph_lower_lev
    
        ! define variable for storing geopotential height for upper level
        double precision :: gph_upper_lev
    
        ! define variable for storing orthometric height for lower level
        double precision :: h_orth_lower_lev
    
        ! define variable for storing orthometric height for upper level
        double precision :: h_orth_upper_lev
    
        ! define variable for storing ellipsoidal height for lower level
        double precision :: h_ell_lower_lev
    
        ! define variable for storing ellipsoidal height for upper level
        double precision :: h_ell_upper_lev
        ...
        ! allocate variables
        allocate( lat_vec_rad(grid_size(1)), &
        ...
        !       it is sufficient to transform only the first column of latitudes to [rad] as this will be only used
        !       for the calculation of g_prep.
        lat_vec_rad= lat_grid(:, 1) * deg2rad ! in [rad]
        ...
        ! get the lowest interval end that is above the minimum station height and the according step width for this interval
        ...
        ! loops to interpolate the vertical profile (meteorological parameters) separately for each horizontal grid point
    
        ! loop over all latitude grid points
        loop_lat: do ind_lat= 1, grid_size(1)
            ...
            ! note: The calculation of g_prep is just dependent on the latitude value. The latitude values in the lat_grid are independent in any 
            !       case of the longitude value, which means that the columns in lat_grid are the same.
            !       Therefore it is not necessary to calculate g_prep newly if the longitude index changes, but only if the latitude index changes!
            !       Thus, a vector lat_vec_rad containing the latitudes in [rad] of only the first column of lat_grid is used for the calculations
            !       to save processing time.
            
            ! prepare gravity calculation
            ! first part: without height dependent term
            g_prep= gn * ( 1 - 0.0026373d0 * cos( 2 * lat_vec_rad(ind_lat) ) + 0.0000059d0 * ( cos( 2 * lat_vec_rad(ind_lat) ) )**2 )
            
        
            ! loop over all longitude grid points
            loop_lon: do ind_lon= 1, grid_size(2)
                ...
                interpolation_loop: do level= 1, nr_pres_lev-1 ! only till nr_pres_lev-1, because level+1 is used in the loop for the upper level
                    ...
                    gph_lower_lev= gph(level, ind_lat, ind_lon)
                    ! get geopotential heights for the upper level
                    gph_upper_lev= gph(level+1, ind_lat, ind_lon)
                    ...
                    ! Transform orthometric heights to ellipsoidal (=geometric) heights
                    ! for the lower level
                    h_ell_lower_lev=h_orth_lower_lev + undulation(ind_lat, ind_lon) ! in [m]
                    ! for the upper level
                    h_ell_upper_lev=h_orth_upper_lev + undulation(ind_lat, ind_lon) ! in [m]
                    ...
                    wvpr_lower_lev= wvpr(level, ind_lat, ind_lon) ! in [hPa]
                    ! get upper level water vapour pressure
                    wvpr_upper_lev= wvpr(level+1, ind_lat, ind_lon) ! in [hPa]
        
                    ! get lower level temperature
                    T_lower_lev= T(level, ind_lat, ind_lon) ! in [K]
                    ! get upper level temperature
                    T_upper_lev= T(level+1, ind_lat, ind_lon) ! in [K]
        
        
                    ! calculate the virtual temperature using meteorological values from the lower level
                    ! necessary for interpolation of the next (higher) interpolation levels                  
                    ...
                    h_ell_mean_uplow= (h_ell_lower_lev + h_ell_upper_lev) / 2
        
        
                    ! refinement of meteorological values received from the grib-file
        
                    ! start loop for interpolation
                    ! --> until the upper level of the grib-file is reached interpolation is done for all height levels below (defined in the variable "h_lev_lowest2upper_limit")
                    inner_interpolation_loop: do while (h_lev_lowest2upper_limit(ind_interp) <= h_ell_upper_lev)
                        ...
                        if (h_lev_lowest2upper_limit(ind_interp) <= h_ell_mean_uplow) then
                            ...
                            ! note: since Fortran 90: two arrays that are conformable (same shape) are processed element-wise in a common expression
                            !                         array and scalar in a statement lead to element-wise calculation of array elements
                            p_int(ind_interp, ind_lat, ind_lon)= p_lower_lev * exp(-(h_lev_lowest2upper_limit(ind_interp) - h_ell_lower_lev) * g_lower / (Rd * Tv_lower_lev)) ! in [hPa]
                        ...
                        ! --> interpolation for the pressure is done using the pressure and virtual temperature values from the upper level
                            ...
                            ! note: since Fortran 90: two arrays that are conformable (same shape) are processed element-wise in a common expression
                            !                         array and scalar in a statement lead to element-wise calculation of array elements
                            p_int(ind_interp, ind_lat, ind_lon)= p_upper_lev * exp(-(h_lev_lowest2upper_limit(ind_interp) - h_ell_upper_lev) * g_upper / (Rd * Tv_upper_lev)) ! in [hPa]
            
                        end if
            
                        ! interpolate the temperature --> linear interpolation
                        T_int(ind_interp, ind_lat, ind_lon)= T_upper_lev + (T_lower_lev - T_upper_lev) / (h_ell_lower_lev - h_ell_upper_lev) * (h_lev_lowest2upper_limit(ind_interp) - h_ell_upper_lev)
                        ...
                        if ( (wvpr_lower_lev == 0) .OR. (wvpr_upper_lev == 0) .OR. (wvpr_lower_lev == wvpr_upper_lev) ) then
                            ...
                            wvpr_int(ind_interp, ind_lat, ind_lon)= wvpr_lower_lev + (wvpr_upper_lev - wvpr_lower_lev) * (h_lev_lowest2upper_limit(ind_interp) - h_ell_lower_lev) / (h_ell_upper_lev - h_ell_lower_lev) ! in [hPa]
                        ...
                        ! for all other cases use logarithmic interpolation
                            ...
                            wvpr_int(ind_interp, ind_lat, ind_lon)= wvpr_lower_lev * exp( (h_lev_lowest2upper_limit(ind_interp) - h_ell_lower_lev) / C) ! in [hPa]

                            ! if the wvpr from the logarithmic interpolation is unrealistically high (> 50 hPa) then perform a linear interpolation instead
                            if (wvpr_int(ind_interp, ind_lat, ind_lon) > 50) then
                                wvpr_int(ind_interp, ind_lat, ind_lon)= wvpr_lower_lev + (wvpr_upper_lev - wvpr_lower_lev) * (h_lev_lowest2upper_limit(ind_interp) - h_ell_lower_lev) / (h_ell_upper_lev - h_ell_lower_lev) ! in [hPa]
                            end if
                        ...
                        rho_d_int= ( p_int(ind_interp, ind_lat, ind_lon) - wvpr_int(ind_interp, ind_lat, ind_lon) ) / ( Rd * T_int(ind_interp, ind_lat, ind_lon) ) ! in [hPa*s^2/m^2]
                        ...
                        rho_w_int= wvpr_int(ind_interp, ind_lat, ind_lon) / ( Rw * T_int(ind_interp, ind_lat, ind_lon) ) ! in [hPa*s^2/m^2]
                        ...
                        Zw_inv_int= 1 + wvpr_int(ind_interp, ind_lat, ind_lon) * ( 1 + 3.7d-4 * wvpr_int(ind_interp, ind_lat, ind_lon) ) * ( -2.37321d-3 + 2.23366d0 / T_int(ind_interp, ind_lat, ind_lon) - 710.792d0 / ( T_int(ind_interp, ind_lat, ind_lon) **2 ) + 7.75141d4 / ( T_int(ind_interp, ind_lat, ind_lon) **3 ) )
                        ...
                        n_h_int(ind_interp, ind_lat, ind_lon)= rho_d_int + rho_w_int
                        ! calculate the wet refractive index (first step: only assign Zw_inv)
                        n_w_int(ind_interp, ind_lat, ind_lon)= Zw_inv_int
        ...
        ! determine the water vapour pressure for the extrapolation height levels
        ...
        extrapolation_loop: do ind_interp= (nr_h_lev_lowest2ECMWF + 1), nr_h_lev_lowest2upper_limit
            ...
            ! assign value of "wvpr_extrap" to whole grid of current height level
            wvpr_int(ind_interp, :, :)= wvpr_extrap ! in [hPa]
            
            ! calculate the density of dry air for the extrapolation height level, [Md/R=1/Rd=Rd^-1]
            ! see scriptum Atmospheric Effects in Geodesy 2012, equation (2.53) on page 34
            rho_d_int= ( p_extrap - wvpr_extrap ) / ( Rd * T_extrap ) ! in [hPa*s^2/m^2]
            ! calculate the density of water vapour for the extrapolation height level, [Mw/R=1/Rw=Rw^-1]
            ! see scriptum Atmospheric Effects in Geodesy 2012, equation (2.54) on page 34
            rho_w_int= wvpr_extrap / ( Rw * T_extrap ) ! in [hPa*s^2/m^2]
            
            ! calculate inverse compressibility factor of water vapour for the extrapolation height level
            ! see Kleijer 2004, Troposphere Modeling and Filtering for Precise GPS Leveling, equation  (4.31) on page 28
            Zw_inv_int= 1 + wvpr_extrap * ( 1 + 3.7d-4 * wvpr_extrap ) * ( -2.37321d-3 + 2.23366d0 / T_extrap - 710.792d0 / ( T_extrap**2 ) + 7.75141d4 / ( T_extrap**3 ) )
            
            ! calculate the refractive index for the extrapolation height level
            ! see Kleijer 2004, Troposphere Modeling and Filtering for Precise GPS Leveling, equation  (4.36) on page 29
            ! calculate the hydrostatic refractive index, [R/Md=Rd] (first step: only total rho is assigned)
            ! note: Scalar value of expression result is assigned to whole grid as the value is independent of latitude or longitude position
            !       when using the US Standard Atmosphere 1976.
            n_h_int(ind_interp, :, :)= rho_d_int + rho_w_int
            ! calculate the wet refractive index (first step: only assign Zw_inv)
            ! note: Scalar value of expression result is assigned to whole grid as the value is independent of latitude or longitude position
            !       when using the US Standard Atmosphere 1976.
            n_w_int(ind_interp, :, :)= Zw_inv_int
        ...
        n_w_int= 1 + ( n_w_int * ( k2s * wvpr_int / T_int + k3 * wvpr_int / (T_int**2) ) ) * 1d-6
    ...
    end subroutine profilewise_refrHD_ECMWFmin
    
end module module_profilewise_refrHD_ECMWFmin
