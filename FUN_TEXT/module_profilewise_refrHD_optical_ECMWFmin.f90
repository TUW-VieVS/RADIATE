! module_profilewise_refrHD_optical_ECMWFmin.f90

!****************************************************************************
!
! PURPOSE:  Module containing subroutine to calculate the optical refractive index profiles for a grid (profilewise approach)
!            
!            "profilewise_refrHD_optical_ECMWFmin" is a subroutine to calculate the optical
...
! History:
! 
! 27.02.2018: create the Fortran-file based on the Fortran module "module_profilewise_refrHD_ECMWFmin.f90"
!             adapt module description
! 08.03.2018: add constants (wavelength, group refractive indices)
!
!****************************************************************************

module module_profilewise_refrHD_optical_ECMWFmin

contains

    subroutine profilewise_refrHD_optical_ECMWFmin( min_stat_h, &
                                            lat_grid, &
                                            grid_size, &
                                            nr_pres_lev, &
                                            gph, &
            ...
            loop_lon: do ind_lon= 1, grid_size(2)
                ...
                interpolation_loop: do level= 1, nr_pres_lev-1 ! only till nr_pres_lev-1, because level+1 is used in the loop for the upper level
                    ...
                    inner_interpolation_loop: do while (h_lev_lowest2upper_limit(ind_interp) <= h_ell_upper_lev)
                        ...
                        ! see Mendes & Pavlis 'High-accuracy zenith delay prediction at optical wavelengths'
                        t_int_deg = T_int(ind_interp, ind_lat, ind_lon) - 273.15d0 ! in [°C]
                        xw = wvpr_int(ind_interp, ind_lat, ind_lon) / p_int(ind_interp, ind_lat, ind_lon)
                        Z_int = 1 - (p_int(ind_interp, ind_lat, ind_lon) / T_int(ind_interp, ind_lat, ind_lon)) * (a0 + a1*t_int_deg + a2*t_int_deg**2 + (b0 + b1*t_int_deg) * xw + (c0 + c1*t_int_deg) * xw**2) + (p_int(ind_interp, ind_lat, ind_lon) / T_int(ind_interp, ind_lat, ind_lon))**2 * (d0 + e0*xw**2)
                        
                        ! calculate the density of water vapour for the interpolation height level
                        ! see Mendes & Pavlis 'High-accuracy zenith delay prediction at optical wavelengths'
                        rho_w_int= (Mw * xw * p_int(ind_interp, ind_lat, ind_lon)) / (R * Z_int * T_int(ind_interp, ind_lat, ind_lon)) ! in [hPa*s^2/m^2]
            
                        ! calculate the total density of air for the interpolation height level
                        ! see Mendes & Pavlis 'High-accuracy zenith delay prediction at optical wavelengths'
                        rho_total_int= Md / (R * Z_int) * (p_int(ind_interp, ind_lat, ind_lon) / T_int(ind_interp, ind_lat, ind_lon) - (1 - Mw/Md) * wvpr_int(ind_interp, ind_lat, ind_lon) / T_int(ind_interp, ind_lat, ind_lon)) ! in [hPa*s^2/m^2]
                        ...
                        n_h_int(ind_interp, ind_lat, ind_lon)= rho_total_int
                        ! calculate the wet refractive index
                        n_w_int(ind_interp, ind_lat, ind_lon)= Ngws * (rho_w_int/rho_ws) - Ngaxs * (Td_k/Pd) * (Mw/Md) * (Zd/Z_int) * (wvpr_int(ind_interp, ind_lat, ind_lon) / T_int(ind_interp, ind_lat, ind_lon))
        ...
        extrapolation_loop: do ind_interp= (nr_h_lev_lowest2ECMWF + 1), nr_h_lev_lowest2upper_limit
            ...
            ! calculate the total density of air for the interpolation height level
            ! see Mendes & Pavlis 'High-accuracy zenith delay prediction at optical wavelengths'
            rho_total_int= Md / (R * Z_int) * (p_extrap / T_extrap - (1 - Mw/Md) * wvpr_extrap / T_extrap) ! in [hPa*s^2/m^2]
            ...
            ! note: Scalar value of expression result is assigned to whole grid as the value is independent of latitude or longitude position
            !       when using the US Standard Atmosphere 1976.
            n_w_int(ind_interp, :, :)= Ngws * (rho_w_int/rho_ws) - Ngaxs * (Td_k/Pd) * (Mw/Md) * (Zd/Z_int) * (wvpr_extrap / T_extrap)
    ...
    end subroutine profilewise_refrHD_optical_ECMWFmin
    
end module module_profilewise_refrHD_optical_ECMWFmin